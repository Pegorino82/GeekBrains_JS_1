...
const config = {
    ...
    getPoinsScreenSelector() {
        return this.settings.pointsScreenSelector
    },
...
const snake = {
    ...
    getNextStepHeadPoint() {
        ...
        switch (this.direction) {
            case 'up':
                return firstPoint.y > 0 ? {x: firstPoint.x, y: firstPoint.y - 1} : {x: firstPoint.x, y: 20};
            case 'right':
                return firstPoint.x < 20 ? {x: firstPoint.x + 1, y: firstPoint.y} : {x: 0, y: firstPoint.y};
            case 'down':
                return firstPoint.y < 20 ? {x: firstPoint.x, y: firstPoint.y + 1} : {x: firstPoint.x, y: 0};
            case 'left':
                return firstPoint.x > 0 ? {x: firstPoint.x - 1, y: firstPoint.y} : {x: 20, y: firstPoint.y};
        }
    },
...
const game = {
    ...
    reset() {
        ...
        /// task 1 мне не очень нравитсячто инициализация карты и в reset и в init игры
        this.map.init(this.config.getRowsCount(), this.config.getColsCount());
    ...
    tickHandler() {
        ...
        if (this.food.isOnPoint(this.snake.getNextStepHeadPoint())) {
            ...
            this.snake.growUp();
            //task 2 если змейка выросла, то начисляем очки
            this.renderPoints();
    ...
    canMakeStep() {
        ...
        //task 3 проверям, что змейка не съела саму себя
    ...
    /**
     * рендерит счет в html
     */
    renderPoints() {
        let pointScreen = document.querySelector(this.config.getPoinsScreenSelector());
        pointScreen.innerText = this.setPoints();
    },

    /**
     * здесь определяется логика начисления очков
     * @return {number} количество очков
     */
    setPoints() {
        return this.count = this.snake.getBody().length - 1;
    }
};
